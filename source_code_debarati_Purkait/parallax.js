...
$(document).ready(function () {
	...
	$(window).scroll(function () {
		...
		if (scroll_measure > 0 && scroll_measure < segment) {
			$('.segment1').css({'background-position':  '0px ' + (-(scroll_measure/2)) + 'px'});
		...
		if (scroll_measure > 100) {
			...
			$(".layer2 img").css({
				'transform': 'translate3d(0px,-' + (scroll_measure * 0.2) + 'px,0px)'
			});
			$(".layer3 img").css({
		...
		if(scroll_measure > 300){
			$(".mermaid2").css({
				'transform': 'translate3d(' + ((scroll_measure) * 0.7) + 'px,-' + ((scroll_measure-400) * 1.5) + 'px,0px)'
			});
		...
		$(".mermaid1-1").show();	
		$(".mermaid1-1").css({
			'transform':'translate3d(' + (-(scroll_measure-500) *1.2) + 'px,'+(-(scroll_measure - 500) * 0.5)+'px,0px)'
		});
		$(".mermaid1").hide();
	}
	else{
		...
		// }
		 if(scroll_measure>1250 && scroll_measure<1506){
		 	$('.turtle').css({
		 ...
		 if(scroll_measure>1483){
			...
			$('.group-of-fish').css({
				'transform':'translate3d('+(scroll_measure-1480)+'px,'+((scroll_measure-1480)*0.5)+'px,0px)'
			});
		 }
		 var movementStrength = 25;
		var height = movementStrength / $(window).height();
		var width = movementStrength / $(window).width();
		$(".segment2").mousemove(function(e){
          var pageX = e.pageX - ($(window).width() / 2);
          var pageY = e.pageY - ($(window).height() / 2);
          var newvalueX = width * pageX * -1 - 25;
          var newvalueY = height * pageY * -1 - 50;
          $('.segment2').css("background-position", newvalueX+"px     "+newvalueY+"px");
});
// 		$(".mermaid").mousemove(function(e){
//           var pageX = e.pageX - ($(window).width() / 2);
//           var pageY = e.pageY - ($(window).height() / 2);
//           var newvalueX = width * pageX * -1 - 25;
//           var newvalueY = height * pageY * -1 - 50;
//           $('.mermaid').css("background-position", newvalueX+"px     "+newvalueY+"px");
// });
